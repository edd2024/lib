...
function generateModulesCSS() {
  ...
  let content = '/* Auto-generated by scripts/build-modules.js — DO NOT EDIT */\n';
  for (const f of files) {
    const css = fs.readFileSync(path.join(SRC_CSS_DIR, f), 'utf8');
    content += `\n/* ---- ${f} ---- */\n${css}\n`;
  }
  fs.writeFileSync(MODULES_CSS_PATH, content, 'utf8');
  console.log(`[build-modules] Wrote ${path.relative(process.cwd(), MODULES_CSS_PATH)} with ${files.length} css files (concatenated)`);
}
